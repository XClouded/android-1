...
public class HttpHeaderFactory {
    ...
    public static String getWeiboAuthHeader(String method, String url, WeiboParameters params,
            String app_key, String app_secret, String token){
        ...
        WeiboParameters authParams = generateAuthParameters(nonce, timestamp, token,app_key);
        // 生成用于计算signature的，参数串
        WeiboParameters signatureParams = generateSignatureParameters(authParams, params, url,app_key);
        // step 3: 生成用于签名的base String
        String oauthBaseString = generateAuthSignature(method, signatureParams, url);
    ...
    private static String generateAuthSignature(final String method, WeiboParameters signatureParams,final String url) {
    ...
    private static WeiboParameters generateSignatureParameters(WeiboParameters authParams,WeiboParameters params, String url,String appKey){
        WeiboParameters signatureParams = new WeiboParameters();
        signatureParams.addAll(authParams);
        signatureParams.add("source", appKey);
    ...
    private static WeiboParameters generateAuthParameters(long nonce, long timestamp, String token,String app_key) {
        ...
        if (token != null) {
            authParams.add("oauth_token", token);
        } else {
            authParams.add("source", app_key);
        }
        return authParams;
    }

    // 生成用于哈希的base string串，注意要按顺序，按需文档需求参数生成，否则40107错误
    public static WeiboParameters generateSignatureList(WeiboParameters bundle) {
        return null;
    }

    // add additional parameters to des key-value pairs,support to expanding
    // params
    public static void addAdditionalParams(WeiboParameters des, WeiboParameters src) {
        // TODO Auto-generated method stub
        
    }

    // 解析url中参数对,存储到signatureBaseParams
    public static void parseUrlParameters(String url, WeiboParameters signatureBaseParams) {
