...
public class DecompressXML {
	...
	public static String decompressXML(byte[] xml) {
		...
		while (off < xml.length) {
			...
			if (tag0 == startTag) { // XML START TAG
				...
				builder.append(prtIndent(indent, "<" + name + sb + ">"));
				indent++;
				...
				String name = compXmlString(xml, sitOff, stOff, nameSi);
				builder.append(prtIndent(indent, "</" + name + ">"));
				// tr.parent(); // Step back up the NobTree

			} else if (tag0 == endDocTag) { // END OF XML DOC TAG
				break;
			} else {
				builder.append(("  Unrecognized tag code '"+ Integer.toHexString(tag0) + "' at offset " + off));
				break;
			}
	...
	public static String prtIndent(int indent, String str) {
		return spaces.substring(0, Math.min(indent * 2, spaces.length()))
				+ str;
	}
}
